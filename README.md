...
- タッチ操作で、グループツールを押したまま展開し、ドラッグして離すことで選択できる操作を追加
- 指が項目間を通過しても最後に選んだ候補を保持し、誤選択を抑制
- マウス操作では従来のホバー展開とクリック選択を維持
- メニューバー、ツウィーク／fウィンドウ、設定画面などのボタンへ沈み込み・反発の視覚効果を追加
- タッチ中の代表ボタン、展開リスト、選択候補を強調表示
- 動きを減らすOS設定ではアニメーションを抑制
- alpha.6までのルート直下公開・完全オフライン構成を維持

## v0.2.0-alpha.6の主な変更

- アレー図・玉・人・まとまりで変更した色、大きさ、不透明度を次の配置へ継承
- 「まとまり」の数値も次の配置へ継承し、段階スライダーへ1000・10000を追加
- 大きさ、行数、列数など主要な数値項目へコンパクトな＋／－ボタンを追加
- ツウィーク／fウィンドウを1組として左右へ配置できる設定を追加
- 4ショット画像の中央へ縦横の分割線を追加
- alpha.5のブランチ直下公開・完全オフライン構成を維持
